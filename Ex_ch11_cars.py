...
def cars_EDA(df):
    ...
    **연비(mpg)와 주요 변수들 간의 관계, 대륙별 특성, 연도별 변화** 등을 탐색합니다.
    ...
    fig5 = px.bar(
        mpg_corr.reset_index(),
        x="index",
        y="mpg",
        color="mpg",
        title="연비(mpg)와 주요 변수들의 상관관계",
        labels={"index": "변수", "mpg": "상관계수"},
    ...
    st.info("""
    💡 **분석 포인트**
    - 상관계수가 음수이면 해당 값이 커질수록 연비가 낮아지는 경향이 있습니다.
    - `weightlbs`, `hp`, `cubicinches`, `cylinders`는 연비와 음의 상관관계를 보입니다.
    - `year`는 시간이 지날수록 차량 연비가 개선되었기 때문에 양의 상관관계를 보입니다.
    """)
